...
uniform samplerCube color; // сэмплер для кубической карты и цвета
...
void main() {
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        if( tmat.z>-0.5 )
        {
            ...
            for( int i=0; i<32; i++ )
            {
                ...
                col += pow( texture( color, rr ).xyz, vec3(2.2) ) * dot(rr,nor) * ds;
            }
            col /= 32.0;										
            ...
            col += 1.8 * (0.04 + 12.0*fre) * occ * pow( texture( color, ref ).xyz, vec3(2.0) ) * rs;
        ...
        else
        {
            ...
            col = pow( texture( color, rd ).xyz, vec3(2.2) );
        }
        tot += col;
#if AA>1
    }
    ...
    //outColor = vec4( texture( color, rd ).xyz, 1.0 );
