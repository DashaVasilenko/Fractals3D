...
float sierpinskiDodecahedron(vec3 pos, out vec4 trapColor) {
    ...
    for(int i = 0; i < iterations; i++) {        
       	...
       	pos -= 2.0*min(0.0, dot(pos, v1))*v1; 
       	pos -= 2.0*min(0.0, dot(pos, v2))*v2; 
       	pos -= 2.0*min(0.0, dot(pos, v3))*v3; 
...
vec4 render(vec3 eye, vec3 dir, vec2 sp ) {
    ...
    // color fractal
        ...
        vec3 albedo = 0.5 + 0.5*cos( 6.2831*trap.y + color);
    #endif
